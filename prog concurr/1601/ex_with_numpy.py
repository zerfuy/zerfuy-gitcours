...
def creation_regions_V2(coin_x, coin_y, width, height, limite_nb_feuilles):
    ...
    if limite_nb_feuilles < 4 or width == 1 or height == 1:
        ...
        deviation_bas_dte = max(sum(std2(coin_x + width // 2, coin_y + height // 2, width - width // 2, 
...
def montrer_image_via_sa_matrice_par_NUMPY() :
    ...
    #plt.show(block=False) # sans effet
    #plt.draw() # change rien   
...
def load_and_creer_mat_pixels_via_NUMPY(nom_fic_in) :
    ...
    # No work ? pourquoi ???
    #if image_.dtype == np.float32: # Si le résultat n'est pas un tableau d'entiers
        #matrice_pixels = np.array((image_ * 255).astype(np.uint8)) 
        
    if image_.dtype == np.float32:
        #matrice_pixels=np.array([[(int(image_[i,j][0]*255), int(image_[i,j][1]*255),int(image_[i,j][2]*255)) \
