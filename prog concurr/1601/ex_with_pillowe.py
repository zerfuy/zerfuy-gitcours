...
# todo : add mutex on pixel rewrite
...
def Decouper_en4(x, y, width, height, threshold_alpha, procCount, parentnode, tree):
    ...
    tree.append(Node(str(color), parent=parentnode))
    if rm < threshold_alpha:  # Affectation de la couleur moyenne à la partition      
        ...
        if procCount.value < 6:
            mesprocs.append(Thread(target=Decouper_en4, args=(x, y, width//2, height//2, threshold_alpha, procCount, parentnode, tree)))
            procCount.value += 1
        else:
            Decouper_en4(x, y, width//2, height//2, threshold_alpha, procCount, parentnode, tree)
        if procCount.value < 6:
            mesprocs.append(Thread(target=Decouper_en4, args=(x + width//2,  y,  width//2,  height//2, threshold_alpha, procCount, parentnode, tree)))
            procCount.value += 1
        else:
            Decouper_en4(x + width//2,  y,  width//2,  height//2, threshold_alpha, procCount, parentnode, tree)
        if procCount.value < 6:
            mesprocs.append(Thread(target=Decouper_en4, args=(x,  y + height//2,  width//2,  height//2, threshold_alpha, procCount, parentnode, tree)))
            procCount.value += 1
        else:
            Decouper_en4(x,  y + height//2,  width//2,  height//2, threshold_alpha, procCount, parentnode, tree)
        
        Decouper_en4(x+width//2,  y+height//2,  width//2, height//2, threshold_alpha, procCount, parentnode, tree)
...
def code_projet_image() :
    ...
    global tree
    tree.append(ognode)
    Decouper_en4(0, 0, width, height, 15, procCount, ognode, tree) # tester avec les seuils différents 3,  10,  15,  20,  ...

    image_.show()

    # for pre, fill, node in RenderTree(ognode):
